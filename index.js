...
function AtImport(options) {
  ...
  return function(styles, result) {
    ...
    var bundle = parseStyles(result,styles,options,state,[]);
...
function parseStyles(
  ...
  statements.forEach(function(stmt) {
    ...
    resolveImportId(result,stmt,options,state)
  })
  ...
  // return SyncPromise.all(statements.map(function(stmt) {
  //   stmt.media = joinMedia(media, stmt.media)
  //
  //   // skip protocol base uri (protocol://url) or protocol-relative
  //   if (stmt.type !== "import" || /^(?:[a-z]+:)?\/\//i.test(stmt.uri)) {
...
function resolveImportId(
  ...
  var resolved = options.resolve(stmt.uri, base, options)
  if (!Array.isArray(resolved)) {
    resolved = [ resolved ]
  }

  var content = resolved.map(function(file) {
  ...
  stmt.children = content.reduce(function(result, statements) {
    if (statements) {
...
function loadImportContent(
  ...
  var content = options.load(filename, options);
  // if (typeof options.transform !== "function") {
  //   return content
  // }
  if (typeof options.transform === "function") {
    var transformed = options.transform(content, filename, options)
    content = typeof transformed === "string" ? transformed : content
  }
